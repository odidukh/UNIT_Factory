...
/*   Updated: 2019/03/19 16:10:48 by oponomar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		init_labels(t_ui_panel *panel, t_objlst *selected)
{
	...
	kiss_label_new(&selected->labels[10], &panel->panel, "Angle:",
					UI_STEP_X + 123, UI_Y + 8 * UI_STEP_Y);
}
...
void			init_cone_ui(t_ui_panel *panel, t_objlst *selected)
{
	selected->ui_form_amount = 11;
	kiss_label_new(&selected->title, &panel->panel, "Cone", UI_STEP_X, UI_Y);
...
void			update_cone_values(t_objlst *selected)
{
	...
	sscanf(selected->entries[10].text, "%lf", &obj->angle);
	edit_cone(selected);
}
...
void			update_cone_entries(t_objlst *selected)
{
	...
	sprintf(selected->entries[10].text, "%.2lf", obj->angle);
}
