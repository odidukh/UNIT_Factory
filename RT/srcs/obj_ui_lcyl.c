...
/*   Updated: 2019/03/21 16:24:26 by oponomar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		init_labels(t_ui_panel *panel, t_objlst *selected)
{
	...
	kiss_label_new(&selected->labels[11], &panel->panel, "H:",
					UI_STEP_X + 150, UI_Y + 8 * UI_STEP_Y);
}
...
static void		init_entries(t_ui_panel *panel, t_objlst *selected)
{
	...
	kiss_entry_new(&selected->entries[11], &panel->panel, 0, "",
					UI_STEP_X + 180, UI_Y + 8 * UI_STEP_Y - 5, 95);
}

void			init_lcyl_ui(t_ui_panel *panel, t_objlst *selected)
{
	selected->ui_form_amount = 12;
	kiss_label_new(&selected->title, &panel->panel, "Limited cylinder",
					UI_STEP_X, UI_Y);
...
void			update_lcyl_values(t_objlst *selected)
{
	...
	sscanf(selected->entries[11].text, "%lf", &obj->height);
	edit_lcyl(selected);
}
...
void			update_lcyl_entries(t_objlst *selected)
{
	...
	sprintf(selected->entries[11].text, "%.2lf", obj->height);
}
