...
int kiss_window_draw(kiss_window *window, SDL_Renderer *renderer)
{
	...
	if (window->decorate)
		kiss_decorate(renderer, &window->rect, kiss_darkgray, kiss_edge);
	return 1;
}
	...
	char *text, int x, int y, int w)
{
	...
	entry->bg = kiss_darkgray;
	entry->normalcolor = kiss_white;
	entry->activecolor = kiss_orange;
	...
	kiss_array *a, int x, int y, int w, int h)
{
	...
	textbox->bg = kiss_gray;
	textbox->textcolor = kiss_white;
	textbox->hlcolor = kiss_darkgray;
	...
	char *text, kiss_array *a, int x, int y, int w, int h)
{
	...
	kiss_entry_new(&combobox->entry, wdw, 0, text, x, y, w);
	...
	if (kiss_textbox_new(&combobox->textbox, &combobox->window, 0,
		a, x, y + combobox->entry.rect.h, w - 15, h) == -1)
		return -1;
